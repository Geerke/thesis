...
fn __velvet_steal__(worker: &mut velvet::VelvetWorker<__Frame__>) {
    ...
    let mut amount_victims = worker.stealers[len - 1].get_amountof_victim();
    if amount_victims == 0 {
        return;
    }
    let mut n = worker.get_random_withself(amount_victims);
    let result_slot = std::sync::Arc::new(std::sync::Mutex::new(None));
    let mut lock = result_slot.lock().unwrap();
    for _ in 0..len {
        if let Some(target) = worker.stealers[len - 1].get_victim(n) {
        ...
        amount_victims = worker.stealers[len - 1].get_amountof_victim();
        if amount_victims == 0 {
            return;
        }
        n = worker.get_random_withself(amount_victims);
    }
