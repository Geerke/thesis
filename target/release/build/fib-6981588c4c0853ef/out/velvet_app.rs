...
fn __velvet_steal__(worker: &mut velvet::VelvetWorker<__Frame__>) {
    ...
    fn select_victim(worker: &velvet::VelvetWorker<__Frame__>, len: usize) -> usize {
        let amount = worker.stealers[len - 1].get_amountof_victim();
        if amount == 0 {
            return worker.get_random(len);
        }
        let n = worker.get_random_withself(amount);
        if let Some(target) = worker.stealers[len - 1].get_victim(n) {
            ...
            return candidates[worker.get_random_withself(candidates.len())];
        }
